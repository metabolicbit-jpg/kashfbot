// ═══════════════════════════════════════════
// 🌱 KashfBot v13.3 — Hardened + Broadcast
// ═══════════════════════════════════════════
...
// v13.3: بازگرداندن اعداد عربی به toEn
...
// v13.3: پیام همگانی
if(st.step==="BROADCAST_MSG"){await setState(db,uid,"BROADCAST_CONFIRM",{text});return sendMsg(env,uid,`📣 <b>پیش‌نمایش پیام همگانی:</b>\n\n${text}\n\nتأیید می‌کنی؟`,{inline_keyboard:[[{text:"✅ ارسال به همه",callback_data:"bc_confirm"},{text:"❌ انصراف",callback_data:"bc_cancel"}]]});}
if(st.step==="BROADCAST_CONFIRM"){await setState(db,uid,"BROADCAST_CONFIRM",{text});return sendMsg(env,uid,`📣 <b>پیش‌نمایش جدید:</b>\n\n${text}`, {inline_keyboard:[[{text:"✅ ارسال به همه",callback_data:"bc_confirm"},{text:"❌ انصراف",callback_data:"bc_cancel"}]]});}
...
if(st.step==="STAKE_AMOUNT"){const n=parseInt(toEn(text));const x=await db.prepare("SELECT * FROM users WHERE user_id=?").bind(uid).first();if(!n||n<10)return sendMsg(env,uid,"❌ حداقل ۰.",CANCEL_KB);if(n>x.balance)return sendMsg(env,uid,`❌ کافی نیست (داری ${x.balance}).`,CANCEL_KB);d.amount=n;await setState(db,uid,"STAKE_PERIOD",d);return sendMsg(env,uid,`🔒 مقدار: <b>${n}</b>\nدوره را انتخاب کن:`,{inline_keyboard:[[{text:"۷ روز (۰.۵٪)",callback_data:"stake_period:7"},{text:"۳۰ روز (۰.۵٪)",callback_data:"stake_period:30"}],[{text:"❌ انصراف",callback_data:"cancel"}]]});}}
...
// v13.3: کوییز با سقف ۲ تلاش
if(data.startsWith("quiz:")){const[,mId,idx]=data.split(":").map(Number);const m=await db.prepare("SELECT * FROM memberships WHERE id=?").bind(mId).first();if(!m||m.user_id!==uid)return;const ch=await db.prepare("SELECT * FROM channels WHERE id=?").bind(m.channel_id).first();if(m.quiz_correct)return edit("✅ قبلاً پاسخ درست داده‌ای.");const attempts=m.quiz_attempts||0;if(attempts>=QUIZ_MAX_ATTEMPTS)return edit(`⛔ سقف تلاش (${faNum(QUIZ_MAX_ATTEMPTS)} بار) تمام شد.\nپاداش کوییز برای این تسک صفر است؛ عضویت و ماندگاری همچنان اعتبار دارند.`);await db.prepare("UPDATE memberships SET quiz_attempts=quiz_attempts+1 WHERE id=?").bind(mId).run();const opts=JSON.parse(ch.quiz_options||"[]");const tier=TIERS[ch.tier]||TIERS.standard;if(opts[idx]===ch.quiz_answer){await db.prepare("UPDATE memberships SET quiz_correct=1 WHERE id=?").bind(mId).run();await addQS(db,mId,40);const paid=await payAction(env,db,m,"QUIZ",tier.quiz,ch);return edit(`✅ درست! <b>+${faNum(paid)}</b> سکه\n📈 امتیاز کیفیت +۴۰`);}return edit(`❌ اشتباه (تلاش ${faNum(attempts+1)} از ${faNum(QUIZ_MAX_ATTEMPTS)}).\nاگر دوباره اشتباه بزنی، پاداش کوییز صفر می‌شود.`);}

// v13.3: پیام همگانی
if(data==="adm_bc"&&isAdmin(env,uid)){await setState(db,uid,"BROADCAST_MSG");return edit("📣 متن پیام همگانی را بفرست:\n(برای همه اعضا در پیوی ارسال می‌شود)");}
if(data==="bc_cancel"&&isAdmin(env,uid)){await clearState(db,uid);return edit("❌ ارسال همگانی لغو شد.");}
if(data==="bc_confirm"&&isAdmin(env,uid)){const st=await getState(db,uid);if(!st)return;const d=JSON.parse(st.data||"{}");const text=d.text||"";if(!text)return edit("❌ متنی نیست.");await clearState(db,uid);const rows=(await db.prepare("SELECT user_id FROM users").all()).results;let ok=0,fail=0;for(const r of rows){if(await isBanned(db,r.user_id))continue;const res=await bale(env,"sendMessage",{chat_id:r.user_id,text,parse_mode:"HTML"});if(res.ok)ok++;else{const r2=await bale(env,"sendMessage",{chat_id:r.user_id,text});if(r2.ok)ok++;else fail++;}}return sendMsg(env,uid,`📣 <b>ارسال همگانی تمام شد</b>\n✅ موفق: ${faNum(ok)} | ❌ ناموفق: ${faNum(fail)}`,MAIN_KB);}
...
// v13.3: تسک‌های فعال + ارسال دوباره کوییز/فوروارد
if(data==="mytasks"){
  ...
  if(!rows.length)return edit("🎁 <b>تسک‌های فعال</b>\n\nتسک بازی نداری.\nاز «🌟 کشف کانال» شروع کن!",{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"profile_back"}]]});
  const summary=[];let sent=0;
  for(const r of rows){const tier=TIERS[r.tier]||TIERS.standard;
    if(r.status==="assigned"){summary.push(`🔘 <b>${r.title}</b> — هنوز عضو نشده‌ای`);
      if(sent<3){await sendMsg(env,uid,`📢 عضو شو: <b>@${r.username}</b>`,{inline_keyboard:[[{text:"✅ عضو شدم",callback_data:"joined:"+r.id},{text:"📢 باز کردن کانال",url:`https://ble.ir/${r.username}`}]]});sent++;}
    }else{
      const pend=[];if(tier.forward&&!r.forward_verified)pend.push("فوروارد");if(tier.quiz&&!r.quiz_correct)pend.push("کوییز");if(!pend.length)pend.push("در انتظار تأیید ۴۸h");
      summary.push(`⏳ <b>${r.title}</b> — ${pend.join("، ")}`);
      if(sent<3){
        if(tier.quiz&&r.quiz_question&&!r.quiz_correct){const opts=JSON.parse(r.quiz_options||"[]");await sendMsg(env,uid,`❓ <b>کوییز مکان‌محور</b> (+${faNum(tier.quiz)} سکه)\n📢 ${r.title}\n🔗 پست مرجع:\n${r.anchor_post_link||"لینک در کانال"}\n📍 پست ${faNum(r.post_distance||3)}ام بعد از مرجع\n\n${r.quiz_question}`,{inline_keyboard:opts.map((o,i)=>[{text:o,callback_data:`quiz:${r.id}:${i}`}])});sent++;}
        if(tier.forward&&!r.forward_verified){await sendMsg(env,uid,`📤 <b>تسک فوروارد</b> (+${faNum(tier.forward)} سکه)\n📢 ${r.title}\nیک پست کانال را به همین بات فوروارد کن.`,{inline_keyboard:[[{text:"📢 باز کردن کانال",url:`https://ble.ir/${r.username}`}]]});sent++;}
      }
    }
  }
  return edit(`🎁 <b>تسک‌های فعال</b>\n\n${summary.join("\n")}\n\n📨 دکمه‌ها/جزئیات به پیوی‌ات ارسال شد.`,{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"profile_back"}]]});
}
...
// v13.3: فقط از assigned قابل گذار + چک مالکیت
if(data.startsWith("joined:")){const mId=parseInt(data.slice(7));const m=await db.prepare("SELECT * FROM memberships WHERE id=?").bind(mId).first();if(!m||m.user_id!==uid)return edit("❌");if(m.status!=="assigned")return edit("🔁 این تسک قبلاً ثبت شده است.\nوضعیت فعلی تو را در «🎁 تسک‌های فعال» ببین.");const ch=await db.prepare("SELECT * FROM channels WHERE id=?").bind(m.channel_id).first();const res=await bale(env,"getChatMember",{chat_id:"@"+ch.username,user_id:uid});if(!["member","creator","administrator"].includes(res.result?.status))return edit("❌ عضو نشده‌ای!",{inline_keyboard:[[{text:"🔄 بررسی",callback_data:"joined:"+mId}]]});const now=new Date(),check=new Date(now.getTime()+RETENTION_HOURS*3600*1000).toISOString();await db.prepare("UPDATE memberships SET status='joined', joined_at=?, check_at=? WHERE id=?").bind(now.toISOString(),check,mId).run();
...
// v13.3: فقط active + حذف کمپین خودِ کاربر
...
// v13.3: تطبیق دقیق بر اساس کانال فورواردشده + بازخورد فوروارد بی‌ربط
async function handleForward(u,env,fwdUser){const db=env.DB,uid=u.message.from.id;
const rows=(await db.prepare("SELECT m.*, c.username, c.tier FROM memberships m JOIN channels c ON m.channel_id=c.id WHERE m.user_id=? AND m.forward_verified=0 AND m.status IN ('joined','rewarded') AND c.username=?").bind(uid,fwdUser).all()).results;
const m=rows[0];
if(!m){const pending=await db.prepare("SELECT 1 FROM memberships m JOIN channels c ON m.channel_id=c.id WHERE m.user_id=? AND m.forward_verified=0 AND m.status IN ('joined','rewarded') AND c.tier IN ('premium','guaranteed')").bind(uid).first();
...
return false;}
const tier=TIERS[m.tier]||TIERS.standard;if(tier.forward<=0)return false;
await db.prepare("UPDATE memberships SET forward_verified=1 WHERE id=?").bind(m.id).run();await addQS(db,m.id,20);const ch=await db.prepare("SELECT * FROM channels WHERE id=?").bind(m.channel_id).first();const paid=await payAction(env,db,m,"FORWARD",tier.forward,ch);await sendMsg(env,uid,`✅ فوروارد تأیید شد! <b>+${faNum(paid)}</b> سکه\n📢 کانال: @${fwdUser}\n📈 امتیاز کیفیت +۲۰`);return true;}
...
// v13.3: اطلاع رشد لحظه‌ای به مالک کمپین
if(ch.owner_id){const ch2=await db.prepare("SELECT acquired,target FROM channels WHERE id=?").bind(ch.id).first();await bale(env,"sendMessage",{chat_id:ch.owner_id,text:`🎉 عضو جدید تأیید شد\n📢 @${ch.username}\n📈 پیشرفت: ${faNum(ch2.acquired)}/${faNum(ch2.target)}`,parse_mode:"HTML"});}}
...
else{await db.prepare("UPDATE memberships SET retention30_verified=0, status='penalized' WHERE id=?").bind(m.id).run();
// v13.3: بازپرداخت واقعی به مالک
await db.prepare("UPDATE users SET balance=MAX(0,balance-?) WHERE user_id=?").bind(tier.join,m.user_id).run();
if(ch.owner_id){await db.prepare("UPDATE users SET balance=balance+? WHERE user_id=?").bind(tier.join,ch.owner_id).run();const ow=await db.prepare("SELECT balance FROM users WHERE user_id=?").bind(ch.owner_id).first();await logTx(db,ch.owner_id,"ESCROW_REFUND",tier.join,ow.balance,`تضمین ch#${ch.id}`);await bale(env,"sendMessage",{chat_id:ch.owner_id,text:`💰 بازپرداخت تضمین: کاربر قبل از ۳۰ روز خارج شد (${faNum(tier.join)} سکه به شما برگشت).`,parse_mode:"HTML"});}}}
...
if(u.message?.text==="/broadcast"&&isAdmin(env,u.message.from.id)){await setState(env.DB,u.message.from.id,"BROADCAST_MSG");return sendMsg(env,u.message.from.id,"📣 متن پیام همگانی را بفرست:",CANCEL_KB);}
