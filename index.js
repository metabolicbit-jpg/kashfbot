// ═══════════════════════════════════════════
//  KashfBot v13.3 — Bugfix + Broadcast
// ═══════════════════════════════════════════
...
// v13.3: بازگرداندن پشتیبانی اعداد عربی
const toEn=s=>String(s||"").replace(/[۰-٠-٩]/g,d=>{const c=d.charCodeAt(0);if(c>=1776&&c<=1785)return c-1776;if(c>=1632&&c<=1641)return c-1632;return d;});
...
async function notifyOwner(env,text){const o=parseInt(env.OWNER_ID||"0");if(o)try{await bale(env,"sendMessage",{chat_id:o,text,parse_mode:"HTML"});}catch(e){}}
...
// v13.3: flow پیام همگانی
if(st.step==="BROADCAST_TEXT"){
  if(text.length>4000)return sendMsg(env,uid,"❌ متن بیش از ۴۰۰۰ کاراکتر است. کوتاه‌تر کن.");
  const allUsers=(await db.prepare("SELECT COUNT(*) c FROM users").first()).c;
  d.broadcast_text=text;
  await setState(db,uid,"BROADCAST_CONFIRM",d);
  return sendMsg(env,uid,`📨 <b>پیش‌نمایش پیام همگانی</b>\n\n${text}\n\n👥 گیرندگان: <b>${faNum(allUsers)}</b> کاربر\n⚠️ این پیام به همه اعضای بات ارسال می‌شود.\n\nتأیید می‌کنی؟`,{inline_keyboard:[[{text:"✅ ارسال به همه",callback_data:"broadcast_send"},{text:"❌ انصراف",callback_data:"cancel"}]]});
}
...
// v13.3: بازیابی تسک‌های فعال
if(data==="active_tasks"){
  const rows=(await db.prepare("SELECT m.*, c.title, c.username, c.tier, c.quiz_question, c.anchor_post_link, c.post_distance FROM memberships m JOIN channels c ON m.channel_id=c.id WHERE m.user_id=? AND m.status IN ('assigned','joined') ORDER BY m.id DESC LIMIT 5").bind(uid).all()).results;
  if(!rows.length)return edit("🎁 <b>تسک فعالی نداری</b>\nاز «🌟 کشف کانال» یک تسک جدید بگیر!",{inline_keyboard:[[{text:" بازگشت",callback_data:"profile_back"}]]});
  const list=rows.map(r=>{const tier=TIERS[r.tier]||TIERS.standard;let tips=[];if(r.status==="assigned")tips.push("✅ عضو شدم را بزن");if(r.status==="joined"){if(tier.forward)tips.push(" یک پست را به بات فوروارد کن");if(tier.quiz&&r.quiz_question)tips.push("❓ کوییز را جواب بده");}return`📢 <b>${r.title}</b> @${r.username}\n${r.status==="assigned"?" در انتظار عضویت":"⏳ در انتظار تأیید"}\n${tips.length?"👉 "+tips.join(" | "):""}`;}).join("\n──────────\n");
  return edit(` <b>تسک‌های فعال تو</b>\n\n${list}`,{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"profile_back"}]]});
}
...
// v13.3: B17 — فقط active + B18 — حذف کمپین خود مالک
...
// v13.3: اجرای همگانی (batch + گزارش نهایی)
async function ctx_broadcast(env,db,text,senderId){
  const users=(await db.prepare("SELECT user_id FROM users").all()).results;
  const total=users.length;let ok=0,fail=0;
  for(const u of users){
    if(u.user_id===senderId)continue;
    if(await isBanned(db,u.user_id))continue;
    try{await bale(env,"sendMessage",{chat_id:u.user_id,text,parse_mode:"HTML"});ok++;}
    catch(e){fail++;}
    // throttle ملایم برای rate limit بله
    if((ok+fail)%50===0)await new Promise(r=>setTimeout(r,1000));
  }
  await notifyOwner(env,`📤 <b>گزارش ارسال همگانی</b>\n👥 کل: ${faNum(total)}\n✅ موفق: ${faNum(ok)}\n❌ خطا: ${faNum(fail)}`);
}
