// ═══════════════════════════════════════════
// 🌱 KashfBot v15.1 — Golden Economy Build (FINAL)
// ═══════════════════════════════════════════
...
const VESTING_PERCENT = 0.5;
const VESTING_DAYS = 30;
const WAKEUP_INTERVAL_DAYS = 7;
...
const IR = iso => {
    ...
    return new Date(d.getTime() + 3.5*3600*1000);
};
...
function faDate(iso){ 
    try { 
        ...
        if(isNaN(jy) || isNaN(jm) || isNaN(jd)) return "نامشخص";
...
function faTime(iso){ 
    try { 
        ...
        return faNum(String(d.getUTCHours()).padStart(2,"0")+":"+String(d.getUTCMinutes()).padStart(2,"0")); 
    } catch(e) { return ""; } 
}
...
const answerCb=async(env,id)=>{ try { return await bale(env,"answerCallbackQuery",{callback_query_id:id}); } catch(e) { return { ok: false }; }};
...
async function notify(env,db,uid,text){try{const u=await db.prepare("SELECT 1 FROM users WHERE user_id=?").bind(uid).first();if(u)await sendMsg(env,uid,text);}catch(e){}}
async function notifyOwner(env,text){const o=parseInt(env.OWNER_ID||"1381797564");if(o)try{await sendMsg(env,o,text);}catch(e){}}
...
function getTier(trust) {
    if (trust >= 80) return { name: 'diamond', mult: 1.3, label: '💎 الماسی' };
    if (trust >= 60) return { name: 'gold', mult: 1.2, label: '🥇 طلایی' };
    if (trust >= 40) return { name: 'silver', mult: 1.1, label: '🥈 نقره‌ای' };
    return { name: 'bronze', mult: 1.0, label: '🥉 برنزی' };
}

async function burnCoins(db, amount) { await db.prepare("UPDATE economy_state SET total_burned = total_burned + ? WHERE id=1").bind(amount).run(); }

async function addVesting(db, uid, amount) {
    const release_at = new Date(Date.now() + VESTING_DAYS * 24 * 60 * 60 * 1000).toISOString();
...
async function releaseVesting(env, db, uid) {
    ...
    if (rows.length) {
        ...
        for (const v of rows) {
            total += v.amount;
            await db.prepare("DELETE FROM vesting WHERE user_id=? AND release_at=?").bind(uid, v.release_at).run();
        }
        await db.prepare("UPDATE users SET balance = balance + ? WHERE user_id=?").bind(total, uid).run();
        const bal = (await db.prepare("SELECT balance FROM users WHERE user_id=?").bind(uid).first()).balance;
        await sendMsg(env, uid, `🔓 ${faNum(total)} سکه از صندوق وفاداری آزاد شد! موجودی: ${faNum(bal)}`);
    }
}
...
async function payAction(env,db,m,action,base_coins,ch){
    if(base_coins<=0||!ch||ch.budget_coins<base_coins)return 0;
    const user = await db.prepare("SELECT trust_score FROM users WHERE user_id=?").bind(m.user_id).first();
    const tier = getTier(user.trust_score || 50);
    const final_coins = Math.floor(base_coins * tier.mult);
    const immediate = Math.floor(final_coins * (1 - VESTING_PERCENT));
    const locked = final_coins - immediate;
    await db.prepare("UPDATE users SET balance=balance+? WHERE user_id=?").bind(immediate, m.user_id).run();
    await addVesting(db, m.user_id, locked);
    await db.prepare("UPDATE channels SET budget_coins=budget_coins-?, acquired=acquired+1 WHERE id=?").bind(final_coins, ch.id).run();
    await db.prepare("UPDATE economy_state SET total_locked=total_locked-? WHERE id=1").bind(final_coins).run();
    const u=await db.prepare("SELECT balance FROM users WHERE user_id=?").bind(m.user_id).first();
    await logTx(db, m.user_id, "TASK_"+action, immediate, u.balance, `ch#${ch.id} + ${locked} قفل`);
    return final_coins;
}

const addQS=(db,mId,pts)=>db.prepare("UPDATE memberships SET quality_score=quality_score+? WHERE id=?").bind(pts,mId).run();
async function refundEscrow(env,db,chId,reason=""){const ch=await db.prepare("SELECT * FROM channels WHERE id=?").bind(chId).first();if(!ch||!ch.owner_id||ch.budget_coins<=0)return;const r=ch.budget_coins;await db.prepare("UPDATE users SET balance=balance+? WHERE user_id=?").bind(r,ch.owner_id).run();await db.prepare("UPDATE channels SET budget_coins=0 WHERE id=?").bind(chId).run();await db.prepare("UPDATE economy_state SET total_locked=total_locked-? WHERE id=1").bind(r).run();const u=await db.prepare("SELECT balance FROM users WHERE user_id=?").bind(ch.owner_id).first();await logTx(db,ch.owner_id,"ESCROW_REFUND",r,u.balance,reason);await bale(env,"sendMessage",{chat_id:ch.owner_id,text:`💰 ${faNum(r)} سکه باقی‌مانده کمپین «${ch.title}» برگشت.\n📝 ${reason}`,parse_mode:"HTML"});}
...
async function handleStart(u,env){
  ...
  await releaseVesting(env, db, uid);
...
async function handleStateText(u,env,st){
  ...
  if(st.step==="ADMIN_REPLY"){
    if(!text)return sendMsg(env,uid,"❌ پیام خالی است.");
    await clearState(db,uid);
    const targetUid=d.targetUid;
    await notify(env,db,targetUid,`📩 پاسخ پشتیبانی:\n\n${text}`);
    return sendMsg(env,uid,"✅ پاسخ شما ارسال شد.");
  }
  ...
  if(st.step==="MISSION_REWARD"){const r=parseInt(toEn(text));if(!r||r<1)return sendMsg(env,uid,"❌ عدد معتبر بفرست.",CANCEL_KB);d.m_reward=r;await setState(db,uid,"MISSION_EXPIRE",d);return sendMsg(env,uid,"⏰ زمان انقضا را انتخاب کن:",{inline_keyboard:[[{text:"♾ بدون انقضا",callback_data:"mexp:0"},{text:"۲۴ ساعت",callback_data:"mexp:24"}],[{text:"۷ روز",callback_data:"mexp:168"},{text:"۳۰ روز",callback_data:"mexp:720"}],[{text:"❌ انصراف",callback_data:"cancel"}]]});}
...
if(data.startsWith("quiz:")){
    ...
    if(!ch)return;
    if(m.quiz_correct)return edit("✅ قبلاً پاسخ درست داده‌ای.");
    ...
    const newAttempts=(m.quiz_attempts||0)+1;
    await db.prepare("UPDATE memberships SET quiz_attempts=? WHERE id=?").bind(newAttempts,mId).run();
    ...
    if(opts[idx]===ch.quiz_answer){
        ...
        await edit(`✅ درست! +${faNum(paid)} سکه\n📈 امتیاز کیفیت +۴۰`);

        // ✅ ارسال خودکار تسک فوروارد
        if(tier.forward && !m.forward_verified){
            await sendMsg(env,uid,`📤 <b>تسک فوروارد (+${faNum(tier.forward)} سکه)</b>\n\n۱) کانال @${ch.username} را باز کن\n۲) یکی از پست‌ها را انتخاب کن\n۳) همان پست را به همین بات فوروارد کن`,{inline_keyboard:[[{text:"📢 باز کردن کانال",url:`https://ble.ir/${ch.username}`}]]});
        }
        return;
    }

    // ✅ پاسخ اشتباه
    const remaining=QUIZ_MAX_ATTEMPTS-newAttempts;
    if(remaining > 0){
        return edit(`❌ اشتباه. ${faNum(remaining)} تلاش باقی مانده.`);
    }

    // ✅ اتمام تلاش‌ها — ارسال فوروارد
    await edit(`❌ اشتباه. تلاش‌های شما تمام شد و پاداش کوییز از دست رفت.`);
    if(tier.forward && !m.forward_verified){
        await sendMsg(env,uid,`📤 <b>تسک فوروارد (+${faNum(tier.forward)} سکه)</b>\n\n۱) کانال @${ch.username} را باز کن\n۲) یکی از پست‌ها را انتخاب کن\n۳) همان پست را به همین بات فوروارد کن`,{inline_keyboard:[[{text:"📢 باز کردن کانال",url:`https://ble.ir/${ch.username}`}]]});
    }
}
...
if(data.startsWith("sup_reply:")&&isAdmin(env,uid)){const targetUid=parseInt(data.slice(10));await setState(db,uid,"ADMIN_REPLY",{targetUid});return edit(`✍️ پاسخ به کاربر ${targetUid}\n\nمتن پاسخ را بنویس:`);}
...
if(data==="adm_backup"&&isAdmin(env,uid))return handleAdminBackup(env,uid);
...
if(data.startsWith("adm_resume:")&&isAdmin(env,uid)){await db.prepare("UPDATE channels SET status='active', bot_is_admin=1 WHERE id=?").bind(parseInt(data.slice(11))).run();return handleAdminChannels(env,uid);}
...
if(data.startsWith("stake_period:")){const days=parseInt(data.split(":")[1]);const st=await getState(db,uid);if(!st)return;const d=JSON.parse(st.data||"{}");const amount=d.amount||0;const result=await db.prepare("UPDATE users SET balance=balance-?, staked=staked+? WHERE user_id=? AND balance>=?").bind(amount,amount,uid,amount).run();if(result.meta.changes===0){await clearState(db,uid);return edit("❌ موجودی کافی نیست.");}const now=new Date();const unlock=new Date(now.getTime()+days*86400000).toISOString();await db.prepare("UPDATE economy_state SET total_locked=total_locked+? WHERE id=1").bind(amount).run();await db.prepare("INSERT INTO stakes (user_id,amount,start_at,unlock_at) VALUES (?,?,?,?)").bind(uid,amount,now.toISOString(),unlock).run();const tk=Math.min(Math.floor(amount/10),30);for(let i=0;i<tk;i++)await db.prepare("INSERT INTO lottery_tickets (user_id) VALUES (?)").bind(uid).run();await logTx(db,uid,"STAKE_LOCK",-amount,(await db.prepare("SELECT balance FROM users WHERE user_id=?").bind(uid).first()).balance,`${days} روز`);await clearState(db,uid);return edit(`🔒 ${faNum(amount)} سکه قفل شد (${faNum(days)} روز)\n🎰 ${faNum(tk)} بلیت قرعه‌کشی دریافت کردی!`);}
...
// ✅ تغییر ۳: active_tasks با نمایش لینک مرجع
if(data==="active_tasks"){const rows=(await db.prepare("SELECT m.*, c.title, c.username, c.tier, c.quiz_question, c.quiz_options, c.anchor_post_link, c.post_distance FROM memberships m JOIN channels c ON m.channel_id=c.id WHERE m.user_id=? AND m.status IN ('assigned','joined') ORDER BY m.id DESC LIMIT 10").bind(uid).all()).results;if(!rows.length)return edit("🎁 تسک‌های فعال\n\nتسکی نداری.\nاز «🌟 کشف کانال، گروه و ربات» شروع کن!",{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"profile_back"}]]});const summary=[];let sent=0;for(const r of rows){const tier=TIERS[r.tier]||TIERS.standard;if(r.status==="assigned"){summary.push(`🔘 ${r.title} — هنوز عضو نشده‌ای`);if(sent<3){await sendMsg(env,uid,`📢 ${r.title} — @${r.username}`,{inline_keyboard:[[{text:"✅ عضو شدم",callback_data:"joined:"+r.id},{text:"📢 باز کردن کانال",url:`https://ble.ir/${r.username}`}]]});sent++;}}else{const pend=[];if(tier.forward&&!r.forward_verified)pend.push("فوروارد");if(tier.quiz&&!r.quiz_correct)pend.push("کوییز");if(!pend.length)pend.push("در انتظار تأیید ۴۸h");summary.push(`⏳ ${r.title} — ${pend.join("، ")}`);if(sent<3){if(tier.quiz&&!r.quiz_correct&&r.quiz_question){const opts=JSON.parse(r.quiz_options||"[]");let quizMsg=`❓ <b>کوییز مکان‌محور (+${faNum(tier.quiz)} سکه)</b>\n📢 ${r.title}`;if(r.anchor_post_link){quizMsg+=`\n\n🔗 <b>پست مرجع:</b>\n${r.anchor_post_link}\n📍 <b>جستجو:</b> پست ${faNum(r.post_distance||3)}ام بعد از مرجع`;}else{quizMsg+=`\n\n📍 پاسخ را در کانال پیدا کن`;}quizMsg+=`\n\n❓ ${r.quiz_question}`;await sendMsg(env,uid,quizMsg,{inline_keyboard:opts.map((o,i)=>[{text:o,callback_data:`quiz:${r.id}:${i}`}])});sent++;}if(tier.forward&&!r.forward_verified){await sendMsg(env,uid,`📤 تسک فوروارد (+${faNum(tier.forward)} سکه)\n📢 ${r.title}\nیک پست کانال را به همین بات فوروارد کن.`,{inline_keyboard:[[{text:"📢 باز کردن کانال",url:`https://ble.ir/${r.username}`}]]});sent++;}}}}return edit(`🎁 تسک‌های فعال\n\n${summary.join("\n")}\n\n📨 دکمه‌ها و جزئیات به پیوی‌ات ارسال شد.`,{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"profile_back"}]]});}
...
if(data.startsWith("mission:")){
    ...
    const existing=await db.prepare("SELECT * FROM memberships WHERE user_id=? AND channel_id=? ORDER BY id DESC LIMIT 1").bind(uid,chId).first();
    ...
    const checkISO = new Date(Date.now() + RETENTION_HOURS * 3600 * 1000).toISOString();

    let mId;
    if(existing && existing.status==="assigned"){
        await db.prepare("UPDATE memberships SET status='joined', joined_at=?, check_at=? WHERE id=?").bind(nowISO, checkISO, existing.id).run();
        mId = existing.id;
    } else {
        const ins = await db.prepare("INSERT INTO memberships (user_id,channel_id,status,joined_at,check_at) VALUES (?,?,?,?,?)").bind(uid,chId,"joined",nowISO,checkISO).run();
        mId = ins.meta.last_row_id;
    }
    ...
    // ✅ منطق پی‌درپی
    let finalMsg = `✅ عضویت ثبت شد!\n⏳ تأیید ماندگاری تا ${faDate(check)}`;

    if(tier.quiz && ch.quiz_question && !m.quiz_correct){
        // اول کوییز با لینک مرجع
        const opts=JSON.parse(ch.quiz_options||"[]");
        let quizMsg = `❓ <b>کوییز مکان‌محور (+${faNum(tier.quiz)} سکه)</b>\n\n📢 کانال: ${ch.title}`;
        if(ch.anchor_post_link){
            quizMsg += `\n\n🔗 <b>پست مرجع:</b>\n${ch.anchor_post_link}\n📍 <b>جستجو:</b> پست ${faNum(ch.post_distance||3)}ام بعد از پست مرجع را ببین`;
        } else {
            quizMsg += `\n\n📍 پاسخ را در کانال پیدا کن`;
        }
        quizMsg += `\n\n❓ ${ch.quiz_question}`;
        await sendMsg(env,uid,quizMsg,{inline_keyboard:opts.map((o,i)=>[{text:o,callback_data:`quiz:${mId}:${i}`}])});
        finalMsg += `\n\n🎁 اولین تسک: کوییز مکان‌محور به پیوی‌ات ارسال شد.`;
    } else if(tier.forward && !m.forward_verified){
        await sendMsg(env,uid,`📤 <b>تسک فوروارد (+${faNum(tier.forward)} سکه)</b>\n\n۱) کانال @${ch.username} را باز کن\n۲) یکی از پست‌ها را انتخاب کن\n۳) همان پست را به همین بات فوروارد کن`,{inline_keyboard:[[{text:"📢 باز کردن کانال",url:`https://ble.ir/${ch.username}`}]]});
        finalMsg += `\n\n🎁 تسک فوروارد به پیوی‌ات ارسال شد.`;
    } else {
        finalMsg += `\n\n💡 پاداش عضویت شما پس از تأیید ماندگاری ۴۸ ساعته آزاد می‌شود.`;
    }
...
const list=scored0(all,my);const pos=((idx%list.length)+list.length)%list.length;const{c:ch,hit}=list[pos];const isMatch=hit>0;const overlap=Math.round(hit/Math.max(my.length,1)*100);const g=await channelGrade(db,ch.id);const tier=TIERS[ch.tier]||TIERS.standard;const gradeEmoji={A:"🥇",B:"🥈",C:"🥉",D:"🏆"}[g.g]||"🏆";const rewardEmoji=tier.max>=5?"💎":"🪙";
const m=await db.prepare("SELECT status FROM memberships WHERE user_id=? AND channel_id=? ORDER BY id DESC LIMIT 1").bind(uid,ch.id).first();
const badge=m?({joined:"⏳ در انتظار تأیید",rewarded:"✅ انجام شد",assigned:"🔘 شروع نشده",penalized:"⛔ جریمه شد"}[m.status]||""):"";const text=`🌟 <b>${ch.title}</b> (${faNum(pos+1)}/${faNum(list.length)})\n${isMatch?`🎯 تشابه ${faNum(overlap)}٪`:"🌐 عمومی"}\n${gradeEmoji} رتبه: ${g.g}\n${rewardEmoji} پاداش: ${faNum(tier.max)} سکه${badge?"\n"+badge:""}`;await bale(env,"editMessageText",{chat_id:q.message.chat.id,message_id:q.message.message_id,text,parse_mode:"HTML",reply_markup:{inline_keyboard:[[{text:"🚀 شروع مأموریت",callback_data:"mission:"+ch.id}],[{text:"⏮ قبلی",callback_data:"prev:"+(pos-1)},{text:"⏭ بعدی",callback_data:"next:"+(pos+1)},{text:"🚩 گزارش",callback_data:"report:"+ch.id}]]}});}
...
async function handleForward(u,env,fwdUser){const db=env.DB,uid=u.message.from.id;const m=await db.prepare("SELECT m.* FROM memberships m JOIN channels c ON m.channel_id=c.id WHERE m.user_id=? AND m.forward_verified=0 AND m.status IN ('joined','rewarded') AND c.username=? ORDER BY m.id DESC LIMIT 1").bind(uid,fwdUser).first();if(!m){const pending=await db.prepare("SELECT 1 FROM memberships m JOIN channels c ON m.channel_id=c.id WHERE m.user_id=? AND m.forward_verified=0 AND m.status IN ('joined','rewarded') AND c.tier IN ('premium','guaranteed')").bind(uid).first();if(pending)await sendMsg(env,uid,"ℹ️ این فوروارد به هیچ تسک فعال تو مربوط نیست.\nلطفاً پست را از همان کانالی فوروارد کن که تسکش را شروع کرده‌ای.");return false;}const ch=await db.prepare("SELECT * FROM channels WHERE id=?").bind(m.channel_id).first();if(!ch)return false;const tier=TIERS[ch.tier]||TIERS.standard;if(tier.forward<=0)return false;await db.prepare("UPDATE memberships SET forward_verified=1 WHERE id=?").bind(m.id).run();await addQS(db,m.id,20);const paid=await payAction(env,db,m,"FORWARD",tier.forward,ch);await sendMsg(env,uid,`✅ فوروارد تأیید شد! +${faNum(paid)} سکه\n📢 کانال: @${ch.username}\n📈 امتیاز کیفیت +۲۰`);return true;}
async function handlePayment(u,env){const db=env.DB;const sp=u.message.successful_payment;const pay=await db.prepare("SELECT * FROM payments WHERE payload=?").bind(sp.invoice_payload).first();if(!pay||pay.status==="paid")return;const toman=sp.total_amount/TOMAN_TO_RIAL;const coins=await mintPurchase(db,u.message.from.id,toman);await db.prepare("UPDATE payments SET status='paid', coins_granted=?, bale_transaction_id=? WHERE payload=?").bind(coins,sp.provider_payment_charge_id||"",sp.invoice_payload).run();await sendMsg(env,u.message.from.id,`💎 خرید موفق! +${faNum(coins)} سکه`,MAIN_KB);}
async function handleAdminBackup(env,uid){const db=env.DB;const tables = ["users","transactions","channels","memberships","reports","stakes","lottery_tickets","missions","economy_state", "vesting"];let backup = {};try {for (const t of tables) {const data = (await db.prepare(`SELECT * FROM ${t}`).all()).results || [];backup[t] = data;}const backupStr = JSON.stringify(backup, null, 2);const maxLen = 4000;if (backupStr.length <= maxLen) {return sendMsg(env, uid, `📦 <b>بکاپ کامل دیتابیس</b>\n\n<code>${backupStr}</code>`);} else {for (let i = 0; i < backupStr.length; i += maxLen) {const chunkText = backupStr.substring(i, i + maxLen);await sendMsg(env, uid, `📦 <b>بکاپ (قسمت ${Math.floor(i/maxLen)+1})</b>\n\n<code>${chunkText}</code>`);await new Promise(r => setTimeout(r, 1000));}return;}} catch(e) {return sendMsg(env, uid, "❌ در فرآیند بکاپ خطایی رخ داد.");}}
async function handleAdmin(env,uid){const db=env.DB;const users=await db.prepare("SELECT COUNT(*) c FROM users").first();const act=await db.prepare("SELECT COUNT(*) c FROM channels WHERE status='active'").first();const e=await getEconomy(db);return sendMsg(env,uid,`🛡 پنل ادمین\n👥 کاربران: ${faNum(users.c)}\n📢 فعال: ${faNum(act.c)}\n💰 پشتوانه: ${faNum(Math.round(e.pool_value))} ت\n🪙 سوخته: ${faNum(e.total_burned)} سکه 🔥\n💎 قیمت: ${faNum(Math.round(currentPrice(e)))} ت`,{inline_keyboard:[[{text:"👥 کاربران",callback_data:"adm_users"},{text:"📢 کمپین‌ها",callback_data:"adm_cams"}],[{text:"🎯 مأموریت‌ها",callback_data:"adm_missions"},{text:"📈 کیفیت",callback_data:"adm_eng"}],[{text:"💳 تراکنش‌ها",callback_data:"adm_txs"},{text:"🚩 گزارش‌ها",callback_data:"adm_rpts"}],[{text:"💰 اقتصاد",callback_data:"adm_econ"},{text:"📊 سهمیه",callback_data:"adm_quota"}],[{text:"📣 پیام همگانی",callback_data:"adm_bc"},{text:"📦 بکاپ",callback_data:"adm_backup"}],[{text:"🔄 تازه‌سازی",callback_data:"adm_menu"}]]});}
async function handleAdminUsers(env,uid){const db=env.DB;const rows=(await db.prepare("SELECT * FROM users ORDER BY created_at DESC LIMIT 15").all()).results;const owner=parseInt(env.OWNER_ID||"1381797564");return sendMsg(env,uid,`👥 ۱۵ کاربر آخر\n\n${rows.map((u,i)=>`${faNum(i+1)}. ${u.first_name||"—"}\n🆔 ${u.user_id} 🪙${faNum(u.balance)} 🛡${faNum(u.trust_score)}`).join("\n──────────\n")}`,{inline_keyboard:rows.filter(u=>u.user_id!==owner).map(u=>[{text:`⛔ ${u.first_name||u.user_id}`,callback_data:"adm_ban:"+u.user_id}]).concat([[{text:"🔙 بازگشت",callback_data:"adm_menu"}]])});}
async function handleAdminChannels(env,uid){const db=env.DB;const rows=(await db.prepare("SELECT * FROM channels ORDER BY id DESC LIMIT 15").all()).results;const list=[];for(const ch of rows){const g=await channelGrade(db,ch.id);list.push(`${ch.status==="active"?"🟢":ch.status==="paused"?"🟡":""}#${faNum(ch.id)} 🏆${g.g} ${ch.title}\n📊${faNum(ch.acquired)}/${faNum(ch.target)} 💰${faNum(ch.budget_coins)}`);}return sendMsg(env,uid,`📢 کمپین‌ها\n\n${list.join("\n──────────\n")}`,{inline_keyboard:rows.filter(c=>c.status!=="removed").flatMap(ch=>{const r=[];if(ch.status==="active")r.push({text:`⏸${ch.id}`,callback_data:"adm_pause:"+ch.id});if(ch.status==="paused")r.push({text:`▶${ch.id}`,callback_data:"adm_resume:"+ch.id});r.push({text:`❌${ch.id}`,callback_data:"adm_remove:"+ch.id});return[r];}).concat([[{text:"🔙 بازگشت",callback_data:"adm_menu"}]])});}
async function handleAdminMissions(env,uid){const db=env.DB;await seedMissions(db);const rows=(await db.prepare("SELECT * FROM missions WHERE status='active' ORDER BY id DESC LIMIT 12").all()).results;const list=[];const kb=[];const now=new Date().toISOString();for(const m of rows){const c=await db.prepare("SELECT COUNT(*) c FROM mission_claims WHERE mission_id=?").bind(m.id).first();const active=(!m.start_at||m.start_at<=now)&&(!m.expire_at||m.expire_at>now);list.push(`#${faNum(m.id)} ${active?"🟢":"⏸"} ${m.title}\n🎁 ${faNum(m.reward)} | 👥 انجام: ${faNum(c.c)} | نوع: ${m.type}\n⏳ انقضا: ${m.expire_at?faDate(m.expire_at):"ندارد"}`);kb.push([{text:`❌ حذف #${m.id}`,callback_data:"mdel:"+m.id}]);}kb.push([{text:"➕ مأموریت جدید",callback_data:"adm_mnew"},{text:"🔙 بازگشت",callback_data:"adm_menu"}]);return sendMsg(env,uid,`🎯 مدیریت مأموریت‌ها\n«👥 انجام» = تعداد کاربرانی که پاداش گرفته‌اند.\n\n${list.join("\n──────────\n")||"مأموریتی نیست."}`,{inline_keyboard:kb});}
async function handleAdminEngage(env,uid){const db=env.DB;const rows=(await db.prepare("SELECT * FROM channels ORDER BY id DESC LIMIT 10").all()).results;const list=[];for(const ch of rows){const s=await db.prepare("SELECT COUNT(*) c, SUM(forward_verified) f, SUM(quiz_correct) q, AVG(quality_score) a FROM memberships WHERE channel_id=? AND status='rewarded'").bind(ch.id).first();const g=await channelGrade(db,ch.id);list.push(`🏆${g.g} ${ch.title}\n👥${faNum(s.c)} | 📤فوروارد ${faNum(s.f||0)} | ❓کوییز ${faNum(s.q||0)}\n📈 QS میانگین: ${faNum(Math.round(s.a||0))}`);}return sendMsg(env,uid,`📈 کیفیت کمپین‌ها\n\n${list.join("\n──────────\n")||"داده‌ای نیست"}`,{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"adm_menu"}]]});}
async function handleAdminTxs(env,uid){const db=env.DB;const rows=(await db.prepare("SELECT * FROM transactions ORDER BY id DESC LIMIT 20").all()).results;return sendMsg(env,uid,`💳 ۲۰ تراکنش آخر\n\n${rows.map(t=>`#${faNum(t.id)} ${t.type} ${t.user_id}\n${faNum(t.amount)}→${faNum(t.balance_after)}`).join("\n──────────\n")}`,{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"adm_menu"}]]});}
async function handleAdminReports(env,uid){const db=env.DB;const rows=(await db.prepare("SELECT * FROM reports WHERE target_kind!='ban' ORDER BY id DESC LIMIT 15").all()).results;if(!rows.length)return sendMsg(env,uid,"✅ گزارشی نیست.",{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"adm_menu"}]]});const catIcon=c=>c==="technical"?"⚙️":"⚠️";return sendMsg(env,uid,`🚩 گزارش‌ها\n✅ حل | ⏸ توقف | ❌ حذف\n\n${rows.map(r=>`${catIcon(r.category||"content")}#${faNum(r.id)} ${r.target_kind}#${r.target_id}\n👤${r.reporter_id} 📝${r.reason}\n📅${faDate(r.created_at)}`).join("\n──────────\n")}`,{inline_keyboard:rows.filter(r=>r.status==="open").map(r=>[{text:`✅ حل #${r.id}`,callback_data:"adm_resolve:"+r.id},{text:`⏸ #${r.id}`,callback_data:"adm_rpause:"+r.id},{text:`❌ #${r.id}`,callback_data:"adm_rremove:"+r.id}]).concat([[{text:"🔙 بازگشت",callback_data:"adm_menu"}]])});}
async function handleAdminEconomy(env,uid){const db=env.DB;const e=await getEconomy(db);const esc=await db.prepare("SELECT COALESCE(SUM(budget_coins),0) s FROM channels WHERE status='active'").first();const stk=await db.prepare("SELECT COALESCE(SUM(amount),0) s FROM stakes WHERE status='active'").first();const price = Math.round(currentPrice(e));const msg = `📊 <b>گزارش اقتصادی کشف</b>\n\n💎 <b>قیمت لحظه‌ای:</b> ${faNum(price)} تومان\n\n━━━━━━━━━━━━━━━\n💰 <b>وضعیت پشتوانه</b>\n• پشتوانه (Pool): ${faNum(Math.round(e.pool_value))} ت\n• کارمزد مالک: ${faNum(Math.round(e.weekly_commission))} ت\n• بودجه پاداش: ${faNum(Math.round(e.reward_budget))} ت\n\n━━━━━━━━━━━━━━━\n🪙 <b>عرضه و تقاضا</b>\n• ضرب‌شده: ${faNum(e.total_minted)} سکه\n• سوخته‌شده (Burn): ${faNum(e.total_burned)} سکه 🔥\n• قفل‌شده: ${faNum(e.total_locked)} سکه\n• در گردش: ${faNum(circulating(e))} سکه\n\n━━━━━━━━━━━━━━━\n🔒 <b>سرمایه‌گذاری فعال</b>\n• سپرده کمپین‌ها: ${faNum(esc.s)} سکه\n• استیک کاربران: ${faNum(stk.s)} سکه`;return sendMsg(env,uid,msg,{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"adm_menu"}]]});}
async function handleAdminQuota(env,uid){const db=env.DB;const date=new Date().toISOString().slice(0,10);const q=await db.prepare("SELECT requests FROM quota WHERE date=?").bind(date).first();const req=q?.requests||0;const tables=["users","channels","memberships","transactions","reports","stakes","lottery_tickets","missions","mission_claims"];const counts={};let total=0;for(const t of tables){const r=await db.prepare(`SELECT COUNT(*) c FROM ${t}`).first();counts[t]=r.c;total+=r.c;}return sendMsg(env,uid,`📊 گزارش مصرف و سهمیه\n📅 ${faDate(new Date().toISOString())}\n\n🌐 درخواست امروز: ${faNum(req)} / ${faNum(REQ_LIMIT)}\n✅ مانده: ${faNum(Math.max(REQ_LIMIT-req,0))}\n\n🗄 حجم دیتابیس: ${faNum(total)} رکورد\n💾 ظرفیت D1 رایگان: ۵ گیگابایت`,{inline_keyboard:[[{text:"🔙 بازگشت",callback_data:"adm_menu"}]]});}
async function ctx_broadcast(env,db,text,senderId){const users=(await db.prepare("SELECT user_id FROM users").all()).results;const total=users.length;let ok=0,fail=0;for(const u of users){if(u.user_id===senderId)continue;if(await isBanned(db,u.user_id))continue;try{await bale(env,"sendMessage",{chat_id:u.user_id,text,parse_mode:"HTML"});ok++;}catch(e){fail++;}if((ok+fail)%50===0)await new Promise(r=>setTimeout(r,1000)); await new Promise(r=>setTimeout(r,1000));}await notifyOwner(env,`📤 گزارش ارسال همگانی\n👥 کل: ${faNum(total)}\n✅ موفق: ${faNum(ok)}\n❌ خطا: ${faNum(fail)}`);}
async function runCron(env){const db=env.DB,now=new Date().toISOString();const e=await getEconomy(db);const refuel=Math.floor(e.weekly_commission*0.1);if(refuel>0)await db.prepare("UPDATE economy_state SET reward_budget=reward_budget+?, weekly_commission=weekly_commission-? WHERE id=1").bind(refuel,refuel).run();const due=(await db.prepare("SELECT * FROM memberships WHERE status='joined' AND check_at<=?").bind(now).all()).results;for(const m of due){const ch=await db.prepare("SELECT * FROM channels WHERE id=?").bind(m.channel_id).first();if(!ch)continue;if(await isBanned(db,m.user_id))continue;const tier=TIERS[ch.tier]||TIERS.standard;const res=await bale(env,"getChatMember",{chat_id:"@"+ch.username,user_id:m.user_id});if(["member","creator","administrator"].includes(res.result?.status)){await db.prepare("UPDATE memberships SET status='rewarded' WHERE id=?").bind(m.id).run();await addQS(db,m.id,20);const paid=await payAction(env,db,m,"JOIN",tier.join,ch);await db.prepare("UPDATE users SET trust_score=MIN(100,trust_score+5) WHERE user_id=?").bind(m.user_id).run();if(tier.ret30)await db.prepare("UPDATE memberships SET guarantee_until=? WHERE id=?").bind(new Date(Date.now()+30*86400000).toISOString(),m.id).run();await bale(env,"sendMessage",{chat_id:m.user_id,text:`🎉 ماندگاری ۴۸ ساعته تأیید شد! +${faNum(paid)} سکه\n📢 کانال: @${ch.username}`,parse_mode:"HTML"});if(ch.owner_id){const ch2=await db.prepare("SELECT acquired,target FROM channels WHERE id=?").bind(ch.id).first();await notifyOwner(env,`📥 تأیید ماندگاری: کاربر ${m.user_id}\n📢 @${ch.username} | +${faNum(paid)} سکه\n📈 پیشرفت کمپین: ${faNum(ch2.acquired)}/${faNum(ch2.target)}`);}}else{await db.prepare("UPDATE memberships SET status='penalized' WHERE id=?").bind(m.id).run();await db.prepare("UPDATE users SET balance=MAX(0,balance-?), trust_score=MAX(0,trust_score-10) WHERE user_id=?").bind(PENALTY_COINS,m.user_id).run();await burnCoins(db, PENALTY_COINS);
    await bale(env,"sendMessage",{chat_id:m.user_id,text:`⚠️ <b>خروج زودهنگام ثبت شد!</b>\n\n📢 کانال: ${ch.title || "بدون عنوان"} (@${ch.username || "—"})\n🪙 جریمه: −${faNum(PENALTY_COINS)} سکه (سوزانده شد)\n📉 اعتماد: −۱۰\n\n💡 برای جلوگیری از جریمه، تا پایان دوره در کانال بمانید.`,parse_mode:"HTML"});
    const userInfo = await db.prepare("SELECT first_name, username FROM users WHERE user_id=?").bind(m.user_id).first();
    await notifyOwner(env, `⚠️ <b>خروج زودهنگام</b>\n\n👤 کاربر: ${userInfo?.first_name || "—"} (@${userInfo?.username || "—"})\n🆔 شناسه کاربر: <code>${m.user_id}</code>\n📢 کانال: ${ch.title || "بدون عنوان"} (@${ch.username || "—"})\n🆔 شناسه کمپین: #${ch.id}\n🪙 جریمه: ${faNum(PENALTY_COINS)} سکه\n📅 تاریخ: ${faDate(new Date().toISOString())} ${faTime(new Date().toISOString())}`);
}await new Promise(r=>setTimeout(r,1000));}const g30=(await db.prepare("SELECT * FROM memberships WHERE guarantee_until IS NOT NULL AND guarantee_until<=? AND retention30_verified=0 AND status='rewarded'").bind(now).all()).results;for(const m of g30){const ch=await db.prepare("SELECT * FROM channels WHERE id=?").bind(m.channel_id).first();if(!ch)continue;const tier=TIERS[ch.tier]||TIERS.standard;const res=await bale(env,"getChatMember",{chat_id:"@"+ch.username,user_id:m.user_id});if(["member","creator","administrator"].includes(res.result?.status)){await db.prepare("UPDATE memberships SET retention30_verified=1 WHERE id=?").bind(m.id).run();await addQS(db,m.id,20);const paid=await payAction(env,db,m,"RET30",tier.ret30,ch);await db.prepare("UPDATE users SET trust_score=MIN(100,trust_score+10) WHERE user_id=?").bind(m.user_id).run();await bale(env,"sendMessage",{chat_id:m.user_id,text:`🏅 ماندگاری ۳۰ روزه تأیید شد! +${faNum(paid)} سکه\n📢 کانال: @${ch.username}`,parse_mode:"HTML"});await notifyOwner(env,`📥 تأیید ۳۰ روزه: کاربر ${m.user_id}\n📢 @${ch.username} | +${faNum(paid)} سکه`);}else{const refund=tier.join;await db.prepare("UPDATE memberships SET retention30_verified=0, status='penalized' WHERE id=?").bind(m.id).run();await db.prepare("UPDATE users SET balance=MAX(0,balance-?) WHERE user_id=?").bind(refund,m.user_id).run();await db.prepare("UPDATE users SET balance=balance+? WHERE user_id=?").bind(refund,ch.owner_id).run();await burnCoins(db, refund);await db.prepare("UPDATE economy_state SET total_locked=total_locked-? WHERE id=1").bind(refund).run();const ownerBal=await db.prepare("SELECT balance FROM users WHERE user_id=?").bind(ch.owner_id).first();await logTx(db,ch.owner_id,"RET30_REFUND",refund,ownerBal.balance,`کاربر ${m.user_id} قبل از ۳۰ روز خارج شد`);
    const userInfo30 = await db.prepare("SELECT first_name, username FROM users WHERE user_id=?").bind(m.user_id).first();
    await bale(env,"sendMessage",{chat_id:m.user_id,text:`⚠️ <b>شکست در تضمین ۳۰ روزه!</b>\n\n📢 کانال: ${ch.title || "بدون عنوان"} (@${ch.username || "—"})\n🪙 جریمه: −${faNum(refund)} سکه\n📉 اعتماد: کاهش یافت\n\n💡 شما قبل از ۳۰ روز کانال را ترک کردید و تضمین از دست رفت.`,parse_mode:"HTML"});
    await bale(env,"sendMessage",{chat_id:ch.owner_id,text:`💰 <b>بازپرداخت تضمین ۳۰ روزه</b>\n\n👤 کاربر: ${userInfo30?.first_name || "—"} (@${userInfo30?.username || "—"})\n🆔 شناسه کاربر: <code>${m.user_id}</code>\n📢 کانال: ${ch.title || "بدون عنوان"}\n🪙 مبلغ بازگشتی: +${faNum(refund)} سکه\n📅 تاریخ: ${faDate(new Date().toISOString())} ${faTime(new Date().toISOString())}`,parse_mode:"HTML"});}await new Promise(r=>setTimeout(r,1000));}
...
if(u.message?.text==="/broadcast"&&isAdmin(env,u.message.from.id)){await setState(env.DB,u.message.from.id,"BROADCAST_TEXT",{});return sendMsg(env,u.message.from.id,"📨 پیام همگانی\n\nمتن پیام را بفرست (HTML مجاز).\nحداکثر ۴۰۰۰ کاراکتر.",CANCEL_KB);}
